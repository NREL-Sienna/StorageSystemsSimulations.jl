...
\begin{align*}
    ...
    &\rho^{e+} &\text{Storage Surplus penalty at end of target cost [\$/MWh]}\\
    &\rho^{e-} &\text{Storage Shortage penalty at end of target cost [\$/MWh]}\\
    &\rho^{c} &\text{Storage Cycling Penalty [\$/MWh]}\\
    &\rho^{z} &\text{Regularization Terms Penalty}\\
...
\begin{align*}
    ...
    &\text{ss}^{st}_{t}  & \in \{ 0, 1 \} &\quad \text{Charge/Discharge status Storage}\\
    ...
    &e^{st+}  & \in [0, E^{max}_{st}] &\quad \text{Storage Energy Surplus above target}\\
    &e^{st-}  & \in [0, E^{max}_{st}] &\quad \text{Storage Energy Shortage below target}\\
    &c^{ch-}  & \in [0, T C_{st}] &\quad \text{Charging Cycling Shortage}\\
    &c^{ds-}  & \in [0, T C_{st}] &\quad \text{Discharging Cycling Shortage}\\
    &z^{st, ch}_{t} & \in [0, P^{max,ch}_{st}] &\quad \text{Regularization charge variable}\\
    &z^{st, ds}_{t} & \in [0, P^{max,ds}_{st}] &\quad \text{Regularization discharge variable}\\
\end{align*}
```
...
&\text{End of period energy target constraint}&\\
&e^{st}_{T} + e^{st+} - e^{st-} = E^{st}_{T}&\\
&\text{Storage Cycling Constraints}&\\
...
&\text{Complete Ancillary Services Energy Coverage}&\\
...
&\text{Regularization Constraints}&\\
