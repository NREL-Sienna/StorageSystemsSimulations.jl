using Documenter, StorageSystemsSimulations
...
pages["Model Library"] = make_model_library(
    categories=[Topology, StaticInjection, Service, Branch, PSY.DeviceParameter],
    exceptions=[
        PSY.DynamicComponent,
        PSY.ActivePowerControl,
        PSY.ReactivePowerControl,
        PSY.DynamicBranch,
    ],
    manual_additions=Dict(
        "Service" => ["Reserves" => "model_library/reserves.md"],
        "StaticInjection" =>
            ["Regulation Device" => "model_library/regulation_device.md"],
        "DynamicInjection" => [
            "Dynamic Inverter" => "model_library/dynamic_inverter.md",
...
folders = Dict(
    ...
    "Model Developer Guide" =>
        filter(julia_file_filter, readdir("docs/src/model_developer_guide")),
    "Code Base Developer Guide" =>
        filter(julia_file_filter, readdir("docs/src/code_base_developer_guide")),
)
...
makedocs(
    ...
    authors="Sourabh Dalvi, Jose Daniel Lara, Rodrigo Henriquez",
